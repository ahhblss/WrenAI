"""Content formatters per tool. Produce the LLM-facing ``content`` field of the
envelope from raw engine output (pyarrow Table, manifest dicts, recall rows).

Mirrors wren-langchain/_format.py so MCP tool output matches the SDK tools'
output byte-for-byte. Pure functions; safe to call from a worker thread.
"""
