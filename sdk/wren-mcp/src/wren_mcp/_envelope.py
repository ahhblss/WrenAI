"""Envelope construction and error formatting for MCP tool results.

The envelope is the contract between wren-mcp tools and the AI agents calling
them. It mirrors the wren-langchain / wren-pydantic SDK envelope so tool
output stays consistent across every Wren agent integration.

Success: {"ok": True, "content": str, "data": dict, "warnings": list[str]}
Error:   {"ok": False, "content": str, "error": {"code", "phase", "message", "metadata"}}

Error contract: for recoverable SQL errors (parse / plan / execute), tools
return a *success* MCP result containing the error envelope so the agent can
inspect ``error.phase`` and retry. For infrastructure failures (connection,
config, auth), tools raise - the MCP server surfaces those as protocol-level
errors. This split mirrors the wren-pydantic retry/propagate taxonomy.
"""
