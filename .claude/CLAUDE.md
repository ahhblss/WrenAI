...
- **Concurrency**: `engine_lock` (asyncio.Lock) serializes every engine/connector/memory call - they are blocking and not thread-safe. `run_blocked` offloads to a worker thread under the lock, bounded by `state.tool_timeout` (env `WREN_MCP_TOOL_TIMEOUT`, default 120s): a hung call raises `TimeoutError` and releases the lock so the server stays responsive (the worker thread can't be force-killed). Requires `abandon_on_cancel=True` on `anyio.to_thread.run_sync` - anyio shields the worker future by default, which would make the timeout wait for the thread to finish.
