...
This module is the shared seam consumed by the wren-langchain, wren-pydantic,
and wren-mcp SDKs. Tests should monkeypatch the ``list_profiles`` /
``get_active_profile`` names on *this* module path
(``wren.providers.connection``) - patching the SDK shim paths no longer
affects the real logic.
"""
