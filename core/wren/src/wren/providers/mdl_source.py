...
``ProjectMDLSource`` reads ``target/mdl.json`` from a prepared Wren project
directory. Each ``load_manifest()`` call re-reads the file from disk so that
``wren context build`` updates by an external CLI run are picked up on the
next tool invocation without needing ``toolkit.reload()``.

Shared by wren-langchain, wren-pydantic, and wren-mcp.
"""
...
from wren.providers.exceptions import WrenToolkitInitError
