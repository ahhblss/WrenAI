"""Tests for the shared provider trio in wren.providers.

These cover the same behavior the SDK tests do, but exercise the canonical
core module paths directly (``wren.providers.connection`` /
``.mdl_source`` / ``.memory``). SDK tests monkeypatch these same paths.
"""
...
def test_active_profile_resolves_third(monkeypatch, tmp_path):
    """Layer 3: globally active profile used when no explicit and no project config."""
    monkeypatch.setattr(
        "wren.providers.connection.list_profiles",
...
def test_no_active_profile_raises(monkeypatch, tmp_path):
    monkeypatch.setattr(
        "wren.providers.connection.list_profiles",
        lambda: {},
    )
    ...
    with pytest.raises(WrenToolkitInitError, match="no active Wren profile"):
        ProfileConnectionProvider(project_path=tmp_path)


# ── ProjectMDLSource ──────────────────────────────────────────────────


def test_mdl_source_reads_target_mdl_json(tmp_path):
...
def test_mdl_source_picks_up_file_changes_between_calls(tmp_path):
    target = tmp_path / "target"
    target.mkdir()
...
def test_mdl_source_raises_on_missing_target(tmp_path):
    source = ProjectMDLSource(project_path=tmp_path)
    with pytest.raises(WrenToolkitInitError, match="target/mdl.json"):
        source.load_manifest()


def test_mdl_source_normalizes_malformed_json(tmp_path):
    target = tmp_path / "target"
    target.mkdir()
...
def test_qdrant_provider_is_enabled():
    assert QdrantMemoryProvider().enabled is True
